...
class Loss(loss._Loss):
    ...
    def forward(self, input, labels, alpha):
        output = {}
        # import pdb;pdb.set_trace()
        pool_val_s = torch.mean(
            input['val_s'].reshape(input['val_s'].shape[0]//(args.seq_len//2), args.seq_len//2, -1), dim=1)
        pool_val_s_cls = torch.mean(
            input['val_s_cls'].reshape(input['val_s_cls'].shape[0]//(args.seq_len//2), args.seq_len//2, -1), dim=1)
