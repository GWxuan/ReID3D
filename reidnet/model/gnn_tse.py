...
class Net(nn.Module):
    def __init__(self, k=10, emb_dims=512, output_channels=1024):
        ...
        branch = nn.ModuleList([nn.Conv2d(emb_dims*2, emb_dims, kernel_size=1, bias=False) for _ in range(2)])
        self.TSE_Module = TSE(branch=branch, inputs=emb_dims, outputs=emb_dims)
        
        self.linear1_ = nn.Linear(emb_dims*2, 512, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.conv5(x)
        x = x.reshape(batch_size//30, 30, x.size()[1], -1)
        x = self.TSE_Module(x)
        x = x.view(batch_size, -1)
        x = F.leaky_relu(self.bn6(self.linear1_(x)), negative_slope=0.2)
        x = self.dp1(x)
